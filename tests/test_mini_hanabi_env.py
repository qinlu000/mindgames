...
class TestMiniHanabiEnv(unittest.TestCase):
    def test_registered_raw_env_can_reset(self):
        ...
        self.assertEqual(len(env.state.game_state["deck"]), 8)
    ...
    def test_color_hint_updates_positive_and_negative_constraints(self):
        ...
        self.assertEqual(env.state.game_state["info_tokens"], 1)
    ...
    def test_successful_rank3_play_restores_info_token(self):
        ...
        pid = env.state.current_player_id
        env.state.game_state["info_tokens"] = 1
        env.state.game_state["fireworks"]["Green"] = 2
        env.state.game_state["player_hands"][pid][0] = Card("Green", 3)
        env.state.game_state["deck"] = []

        done, _ = env.step("[Play A]")
        self.assertFalse(done)
        self.assertEqual(env.state.game_state["fireworks"]["Green"], 3)
        self.assertEqual(env.state.game_state["info_tokens"], 2)
    ...
    def test_turn_cap_ends_after_twelve_consumed_turns(self):
        from mindgames.envs.MiniHanabi.env import MiniHanabiEnv, Card

        env = MiniHanabiEnv()
        env.reset(num_players=2, seed=0)
        env.state.game_state["deck"] = [Card("Red", 1) for _ in range(20)]
        ...
        self.assertTrue(done)
        self.assertEqual(env.state.turn, 12)
