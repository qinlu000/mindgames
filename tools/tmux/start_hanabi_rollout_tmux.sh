...
for idx in "${!PORT_ARR[@]}"; do
  ...
  cmd="cd \"$ROOT_DIR\" && CUDA_VISIBLE_DEVICES=$gpu MODEL=\"$MODEL\" HOST=\"$HOST\" PORT=$port CONTEXT_MANAGER=\"$CONTEXT_MANAGER\" HANABI_CTX_MAX_TURNS=\"$HANABI_CTX_MAX_TURNS\" HANABI_CTX_KEEP_SYSTEM=\"$HANABI_CTX_KEEP_SYSTEM\" VLLM_MAX_MODEL_LEN=$VLLM_MAX_MODEL_LEN VLLM_MAX_NUM_SEQS=$VLLM_MAX_NUM_SEQS VLLM_USE_ASYNC_ENGINE=$VLLM_USE_ASYNC_ENGINE NCCL_P2P_DISABLE=$NCCL_P2P_DISABLE NCCL_IB_DISABLE=$NCCL_IB_DISABLE bash tools/rollout/rollout_hanabi_gym_simple.sh 2>&1 | tee -a \"$log_file\""
