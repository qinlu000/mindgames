...
HANABI_FIXED_PLAYER_ID="${HANABI_FIXED_PLAYER_ID:-}"
HANABI_FIXED_ADAPTER_PATH="${HANABI_FIXED_ADAPTER_PATH:-}"
HANABI_FIXED_ADAPTER_NAME="${HANABI_FIXED_ADAPTER_NAME:-}"
SWIFT_BIN="${SWIFT_BIN:-}"
...
for idx in "${!PORT_ARR[@]}"; do
  ...
  cmd="cd \"$ROOT_DIR\" && CUDA_VISIBLE_DEVICES=$gpu MODEL=\"$MODEL\" HOST=\"$HOST\" PORT=$port CONTEXT_MANAGER=\"$CONTEXT_MANAGER\" ENABLE_THINKING=\"$ENABLE_THINKING\" MAX_TURNS=\"$MAX_TURNS\" HANABI_CTX_MAX_TURNS=\"$HANABI_CTX_MAX_TURNS\" HANABI_CTX_KEEP_SYSTEM=\"$HANABI_CTX_KEEP_SYSTEM\" HANABI_FIXED_PLAYER_ID=\"$HANABI_FIXED_PLAYER_ID\" HANABI_FIXED_ADAPTER_PATH=\"$HANABI_FIXED_ADAPTER_PATH\" HANABI_FIXED_ADAPTER_NAME=\"$HANABI_FIXED_ADAPTER_NAME\" VLLM_MAX_MODEL_LEN=$VLLM_MAX_MODEL_LEN VLLM_MAX_NUM_SEQS=$VLLM_MAX_NUM_SEQS VLLM_ENABLE_LORA=$VLLM_ENABLE_LORA VLLM_MAX_LORA_RANK=$VLLM_MAX_LORA_RANK VLLM_USE_ASYNC_ENGINE=$VLLM_USE_ASYNC_ENGINE VLLM_ENGINE_KWARGS='$VLLM_ENGINE_KWARGS' SWIFT_BIN=\"$SWIFT_BIN\" NCCL_P2P_DISABLE=$NCCL_P2P_DISABLE NCCL_IB_DISABLE=$NCCL_IB_DISABLE bash tools/rollout/rollout_hanabi_gym_simple.sh 2>&1 | tee -a \"$log_file\""
