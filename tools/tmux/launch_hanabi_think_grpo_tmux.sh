...
ROLLOUT_SESSION="${ROLLOUT_SESSION:-hanabi_rollout_think}"
TRAIN_SESSION="${TRAIN_SESSION:-hanabi_train_think}"

MODEL="${MODEL:-/workspace/models/Qwen3-8B}"
ADAPTERS="${ADAPTERS:-}"
REF_ADAPTERS="${REF_ADAPTERS:-$ADAPTERS}"
RUN_NAME="${RUN_NAME:-hanabi-grpo-think-$(date +%Y%m%d_%H%M%S)}"
...
SOFT_MAX_LENGTH="${SOFT_MAX_LENGTH:-1536}"
...
if [ -z "${ADAPTERS//[[:space:]]/}" ]; then
  echo "ADAPTERS is required. Point it to the think-capable SFT LoRA checkpoint." >&2
  exit 1
fi
...
echo
echo "Hanabi think GRPO started."
