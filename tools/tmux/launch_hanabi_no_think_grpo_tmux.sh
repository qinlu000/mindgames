...
if [ -n "${HANABI_FIXED_ADAPTER_PATH//[[:space:]]/}" ]; then
  if [ -z "${VLLM_ENGINE_KWARGS//[[:space:]]/}" ]; then
    VLLM_ENGINE_KWARGS='{"max_loras":2}'
  fi
  case " $EXTRA_SWIFT_ARGS " in
    *" --vllm_enable_lora "*)
      ;;
    *)
      EXTRA_SWIFT_ARGS="${EXTRA_SWIFT_ARGS:+$EXTRA_SWIFT_ARGS }--vllm_enable_lora true"
      ;;
  esac
fi
...
if [ -n "${HANABI_FIXED_ADAPTER_PATH//[[:space:]]/}" ]; then
  echo "Fixed opponent:  P${HANABI_FIXED_PLAYER_ID:-1} -> ${HANABI_FIXED_ADAPTER_PATH}"
  echo "Fixed name:      ${HANABI_FIXED_ADAPTER_NAME:-hanabi_fixed_opponent}"
  echo "Rollout kwargs:  $VLLM_ENGINE_KWARGS"
fi
echo "Extra args:      $EXTRA_SWIFT_ARGS"
