...
def main() -> int:
    ...
    with in_path.open("r", encoding="utf-8") as f:
        for line_no, line in enumerate(f, start=1):
            ...
            t = obj.get("type", None)
            if t == "step":
                steps_by_ep[ep].append(
                    _compact_step_rec(
                        obj, max_obs_chars=(None if int(args.max_obs_chars) == 0 else int(args.max_obs_chars))
                    )
                )
                meta_by_ep.setdefault(ep, {"env_id": obj.get("env_id", None), "seed": obj.get("seed", None)})
            elif t == "episode_end":
                end_by_ep[ep] = obj
                meta_by_ep.setdefault(ep, {"env_id": obj.get("env_id", None), "seed": obj.get("seed", None)})
