...
class TruthAndDeceptionEnv(Env):
    ...
    def _load_facts(self, data_path: Optional[str]) -> None:
        try:
            if data_path is not None: # Use provided path
                ...
                try:
                    with importlib.resources.files('mindgames.envs.TruthAndDeception').joinpath('facts.json').open('r') as file:
                        self.facts_data = json.load(file)
                except Exception:
                    # Fallback for editable/source runs if package data isn't available.
    ...
    def step(self, action: str) -> Tuple[bool, Info]:
        ...
        if self.state.turn == self.state.max_turns-2:  # check if the guessing phase has started
            self.state.add_observation(message="Now guess which of the two facts are correct by returning '[Fact 1]' or '[Fact 2]'.", observation_type=ObservationType.GAME_MESSAGE) 
